...
This repository supplements the manuscript submitted as a Brief Communication by Moir *et al*, 2024. The aim herein is to provide further methodological detail and simple R commands to allow independent reproduction of results presented within the submission. 
...
### GSE6532
Breast cancer gene expression dataset GSE6532 is freely available from https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE6532 and was obtained using the getGEO function of the R GEOquery package:
...
PAM50 classification of samples within the GSE6532 dataset were conducted using molecular.subtyping function of the R AIMS library. Sample assigments were designated using the whole dataset and assignments confirmed by re-running the classification individually for each batch of samples.  
...
Within dataset GSE6532, only ESR1 expression is adversely affected by ComBat batch correction displays, we see in figures F,G and H (above) that statistically significant differences between batches within PAM50 subtypes are fewer within this dataset. We can see with ESR1 expression however, that PAM50 stratifed ComBat appears to perform most effectively at removing technical differences while retaining biological signal.
